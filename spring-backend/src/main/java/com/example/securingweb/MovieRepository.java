...
public interface MovieRepository extends Neo4jRepository <Movie, String>{
    ...
    @Query("MATCH (n:Movie), (u:User) WHERE n.name = $moviename and u.username = $username CREATE (u)-[r:RATING {rating: $rating}]->(n)")
    ...
    @Query("MATCH (m:Movie), (u:User) WHERE m.name = $moviename AND u.username = $username RETURN exists((m)<-[:LIKED]-(u))")
